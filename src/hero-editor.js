// hero-editor.js — in-game pixel sprite editor for custom heroes
// Manages: hero list view, new-hero flow, full-screen 6-frame pixel editor.

import { PALETTE } from './sprites.js?v=47';
import { PLAYER_SMALL_STAND_R, PLAYER_SMALL_WALK1_R, PLAYER_SMALL_JUMP_R } from './player-sprites.js?v=47';

const CUSTOM_KEY    = 'mario-tablet:custom-heroes';
...
function saveCustomHeroes(heroes) {
  try { localStorage.setItem(CUSTOM_KEY, JSON.stringify(heroes)); } catch {}
}
...
export function createHeroEditor(builtinHeroes, onClose) {
  ...
  function close() { overlay.style.display = 'none'; if (onClose) onClose(customHeroes); }
  ...
  function renderList(box) {
    const title = el('div', 'ced-title', 'HERO EDITOR');
    box.appendChild(title);

    const list = el('div', 'ced-list');

    // Built-ins (read-only)
    list.appendChild(el('div', 'ced-section-hdr', 'Built-in Heroes'));
    for (const ch of builtinHeroes) {
      const row = el('div', 'ced-list-row');
      row.appendChild(el('span', 'ced-hero-name', ch.name));
      row.appendChild(el('span', 'ced-readonly', 'built-in'));
      list.appendChild(row);
    }

    // Custom heroes
    list.appendChild(el('div', 'ced-section-hdr', 'Custom Heroes'));
    if (customHeroes.length === 0) {
      list.appendChild(el('div', 'ced-empty', 'No custom heroes yet.'));
    }
    customHeroes.forEach((ch, i) => {
      const row = el('div', 'ced-list-row');
      row.appendChild(el('span', 'ced-hero-name', ch.name));
      row.appendChild(mkBtn('Edit', () => {
        editingIdx = i; editingHero = JSON.parse(JSON.stringify(customHeroes[i]));
        activeFrame = 'small_stand'; view = 'editor'; render();
      }));
      row.appendChild(mkBtn('Rename', () => {
        const n = prompt('New name:', ch.name);
        if (n && n.trim()) { customHeroes[i].name = n.trim(); saveCustomHeroes(customHeroes); render(); }
      }));
      row.appendChild(mkBtn('Delete', () => {
        if (confirm(`Delete "${ch.name}"?`)) { customHeroes.splice(i, 1); saveCustomHeroes(customHeroes); render(); }
      }));
      list.appendChild(row);
    });
    ...
    footer.appendChild(mkBtn('+ New Hero', () => { view = 'new'; render(); }, 'ced-new-btn'));
  ...
  function renderNew(box) {
    box.appendChild(el('div', 'ced-title', 'NEW HERO'));
    ...
    for (const ch of builtinHeroes)  addOpt(tmplSel, ch.id, ch.name);
    for (const ch of customHeroes)   addOpt(tmplSel, ch.id, ch.name + ' (custom)');
    ...
    footer.appendChild(mkBtn('Create →', () => {
      ...
      editingHero = buildNewHero(name, tmplSel.value);
  ...
  function buildNewHero(name, tmplId) {
    const ch = {
    ...
    if (!tmplId) return ch;
    const bi = builtinHeroes.find(c => c.id === tmplId);
    if (bi) {
      const ov = bi.colors || {};
      ...
      const cu = customHeroes.find(c => c.id === tmplId);
  ...
  function renderEditor(box) {
    ...
    const nameInput = Object.assign(document.createElement('input'), {
      type: 'text', value: editingHero.name, id: 'ced-name-input',
    });
    nameInput.addEventListener('input', () => { editingHero.name = nameInput.value; });
    const saveBtn = mkBtn('💾 Save', () => {
      editingHero.name = nameInput.value.trim() || 'Unnamed';
      if (editingIdx >= 0) customHeroes[editingIdx] = editingHero;
      else customHeroes.push(editingHero);
      saveCustomHeroes(customHeroes);
      view = 'list'; render();
    }, 'ced-save-btn');
  ...
  function renderEditorBody(main) {
    ...
    const frame = editingHero.frames[activeFrame];
    ...
    if (unlockedFrames.length > 0) {
      ...
      for (const target of unlockedFrames) {
        const btn = mkBtn(target.label, () => {
          editingHero.frames[target.id] = JSON.parse(JSON.stringify(frame));
